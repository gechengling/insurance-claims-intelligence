...
| 欺诈类型 | 评分因子（0-10分） | 数据来源 | 触发阈值 | 应对流程 |
|---------|----------------|---------|---------|---------|
| **票据异常** | 发票号重复(3分)+金额异常(2分)+医院不存在(5分) | OCR+医保数据库 | ≥5分转人工 | 票据原件核验 |
| **就诊频率** | 7天内≥3次(3分)+同一医院(2分)+诊断相似(3分) | 理赔历史库 | ≥5分预警 | 医疗记录调查 |
| **关系网络** | 同一医生(2分)+同一IP(3分)+亲属关系(3分) | 关系图谱数据库 | ≥5分预警 | 关系网络扩线 |
| **金额异常** | 单次>5万(2分)+短期累计>20万(3分)+赔付率>90%(3分) | 财务系统 | ≥5分预警 | 资金流向追踪 |
| **时间异常** | 深夜就诊(23-5点)(2分)+节假日突发(2分)+与投保时间重叠(3分) | 理赔系统日志 | ≥5分预警 | 时间线重建 |
...
def fraud_score(claim):
    ...
    return min(score, 10)  # 封顶10分
```

**图数据库反欺诈架构（2026推荐）**：
- **Neo4j**：社区版免费，支持复杂关系遍历，适合中型保险公司（<1000万理赔/年）
- **TigerGraph**：分布式原生图库，支持实时深度遍历，适合大型保险公司（>1000万理赔/年）
- **Amazon Neptune**：全托管，集成AWS生态，适合已上云企业
